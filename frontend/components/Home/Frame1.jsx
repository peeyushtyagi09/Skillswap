...
const Frame1 = () => {
  ...
  const storeTokenAndNavigate = (token) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      storeTokenAndNavigate(data.accessToken);
    } catch (error) {
  ...
  const resendRegisterOtp = async () => {
    ...
    await api.post('/auth/request-register-otp', {
      username: registerInitialData.username,
      email: registerEmail,
      password: registerInitialData.password,
    });
  };
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 relative">
            ...
            {!needOtp ? (
              ...
              <form onSubmit={submitOtp} className="space-y-6">
                <button type="button" onClick={() => setNeedOtp(false)} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">← Back</button>
                ...
                <div>
                  ...
                  <input value={otp} onChange={e => setOtp(e.target.value)} required maxLength={6} className="w-full px-4 py-3 border border-gray-300 rounded-lg" placeholder="123456" />
                </div>
                ...
                <button type="button" onClick={async () => { setOtpError(''); await api.post('/auth/request-email-otp', { email: pendingEmail }); }} className="mt-2 underline">Resend OTP</button>
              </form>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 relative">
            ...
            {!registerNeedOtp ? (
              <form onSubmit={handleRegister} className="space-y-6">
                ...
                <button type="submit" disabled={registerLoading} className="w-full bg-black text-white py-3 rounded-lg font-semibold disabled:opacity-50">{registerLoading ? 'Sending OTP...' : 'Register'}</button>
              </form>
            ) : (
              <form onSubmit={submitRegisterOtp} className="space-y-6">
                <button type="button" onClick={() => { setRegisterNeedOtp(false); setRegisterOtp(''); setRegisterOtpError(''); }} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">← Back</button>
                <div className="text-sm text-gray-600">Enter the OTP sent to your email.</div>
                {registerOtpError && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">{registerOtpError}</div>}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">OTP</label>
                  <input value={registerOtp} onChange={e => setRegisterOtp(e.target.value)} required maxLength={6} className="w-full px-4 py-3 border border-gray-300 rounded-lg" placeholder="123456" />
                </div>
                ...
                <button type="button" onClick={resendRegisterOtp} className="mt-2 underline">Resend OTP</button>
              </form>
            )}
          </div>
        </div>
